...
bun test
bun run build
install -m 755 "$PROJECT_DIR/dist/mattgpt" "$EXECUTABLE"
...
if [[ "$MATTGPT_PLATFORM" == "darwin" ]]; then
  sed \
    ...
    "$PROJECT_DIR/launchd/com.matgra.mattgpt.plist.template" > "$SERVICE_PATH"
  ...
  if [[ "$bootstrapped" != "1" ]]; then
    printf 'MattGPT could not be loaded after three attempts.\n' >&2
    exit 1
  fi
  ...
  if ! command -v systemctl >/dev/null 2>&1; then
    printf 'systemctl is required to install MattGPT as a Linux user service.\n' >&2
    exit 1
  fi
  sed \
    -e "s|__EXECUTABLE__|$EXECUTABLE|g" \
    ...
    "$PROJECT_DIR/systemd/mattgpt.service.template" > "$SERVICE_PATH"
...
printf 'MattGPT installed and started on %s.\n' "$MATTGPT_PLATFORM"
